...
nps_demo_title <- function(title="employee NPS by demographic", pnum1=pnum, color_back=deutser_red, color_word="white", explan_width=55,explan_fsize=18, fsize=50, xcor=.15, ycor_minus = .08, ycor=.52, interval=.1, file_type=".pdf", df=NULL, institute="DCI",
                           explanation = c("Category options with less than 5 respondents were not included so that aggregate scores are meaningful and to ensure anonymity.")) {
  pnum1 <- pnum1 + 1
  explanation = c("Category options with less than 5 respondents were not included so that aggregate scores are meaningful and to ensure anonymity.")
  ...
  plot <- ggdraw(p) +
    draw_label(toupper(title), fontfamily = "Gotham Black", size = fsize, x = xcor,  y = ycor, hjust = 0, color = color_word) +
  ...
  ggsave(paste0(file.plots, str_pad(pnum1, 3, pad = "0"), "nps_demo_title_", file_type), 
