...
appendix_hor <- function(pnum, df, df_long, demo_vars_hor, leg_font_dims = 9, leg_font_quad =9) {
...
#---------------------------------------------- QUADRANT PLOTS ---------------------------------------  
 ...
 df_quad_plot <- df_quad_plot_pre   %>%
   ...
   mutate(dimension = factor(dimension, levels = levels_dim, ordered = TRUE))
      ...
      p <- ggplot(data = df_quad_plot, aes(df_quad_plot[[1]], score, fill = df_quad_plot[[1]])) + 
        ...
        dem_quad_theme +
        theme(legend.text = element_text(size = leg_font_quad,family = "Gotham-Medium")) +
 ...
 for (q in 5) {
     ...
     for (i in 2:3) 
     {
      ...
      plot <- ggdraw(p) + 
        draw_label(paste0(toupper(str_replace(eval(demo_vars_hor), "_"," ")), ": ", eval(quadrants[[q]][[i]][1])), 
